...
const UseWebSiteScanData = create( ( set, get ) => ( {
	...
	getStatus: async () => {
		if ( get().getStatusCalled ) {
			return;
		}

		try {
			const data = {};
			const { wsc_status, token_status, wsc_signup_date, batch_stats } =
				await cmplz_api
					.doAction( 'get_wsc_status', data )
					.then( ( response ) => {
						return response;
					} );
			set( {
	...
	resetWsc: async () => {
		const confirmation = confirm(
			'Are you sure? This will delete all your Website Scan data.'
		);
		if ( confirmation ) {
			try {
				const { result, redirect } = await cmplz_api
					.doAction( 'reset_wsc' )
					.then( ( response ) => {
						return response;
					} );
	...
	enableWsc: async () => {
		try {
			const { updated, wsc_status, token_status } = await cmplz_api
				.doAction( 'enable_wsc' )
				.then( ( response ) => {
					return response;
				} );
	...
	requestActivationEmail: async () => {
		try {
			const response = await cmplz_api.doAction(
