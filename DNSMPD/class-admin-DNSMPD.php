	...
	class cmplz_admin_DNSMPD {
		...
		public function get_datarequests_data( $data, $action, $request ) {
			...
			if ( $action === 'get_datarequests' ) {
				...
				return $data;
			} elseif ( $action === 'delete_datarequests' ) {
				$records = $request->get_param( 'records' );
				foreach ( $records as $record ) {
					$this->delete( $record['ID'] );
				}
				$data = array();
			} elseif ( $action === 'resolve_datarequests' ) {
				$records = $request->get_param( 'records' );
				foreach ( $records as $record ) {
			...
			} elseif ( $action === 'export_datarequests' ) {
				$data       = $request->get_params();
		...
		public function new_datarequests_notice( $warnings ) {
			$warnings['new_datarequest'] = array(
		 ...
		 *
		 * Prefixes a value with a leading single quote when it starts with a
		 * character that spreadsheet applications (Excel, Sheets, LibreOffice)
		 * treat as a formula trigger, so it is opened as text instead of
		 * being executed as a formula.
		 *
		 * @param mixed $value the value to check and, if needed, escape.
		 *
		 * @return mixed
		 */
		...
		private function create_csv_file( $data, $add_header = true ) {
			...
			if ( $add_header ) {
				...
				$headers = array_map( array( $this, 'csv_safe_value' ), $headers );
			...
			if ( is_array( $data ) ) {
				foreach ( $data as $line ) {
					...
					$line[] = $date;
					$line = array_map( array( $this, 'csv_safe_value' ), $line );
